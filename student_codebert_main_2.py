...
from student_codebert_model_2 import StudentBERT
import pandas as pd
# metrics
...
class Example:
    def __init__(self, input_ids, label, group):
...
class TextDataset(Dataset):
    ...
    def apply_smote(self):
        features = [example.input_ids for example in self.examples]
        labels = [example.label for example in self.examples]
        groups = [example.group for example in self.examples]
        ...
        tomek = TomekLinks()
        features_resampled, labels_resampled = tomek.fit_resample(features, labels)


        self.examples = [Example(input_ids=f.tolist(), label=l, group=g) for f, l, g in zip(features_resampled, labels_resampled,groups)]
...
def train(args, train_dataset, model, teacher_model, tokenizer, eval_dataset, cwe_label_map):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            #teacher de logiti
            teacher_logits, loss_ce_teac = teacher_model(input_ids=input_ids, groups=groups, labels=labels, return_logit=True)
            #teacher_logits = teacher_model(input_ids=input_ids, groups=groups, labels=labels, return_logit=True) #接收两个logitis 教师的logitis预测
            soft_label = nn.functional.log_softmax(teacher_logits, dim=-1) #计算soft_label
            probs = torch.softmax(teacher_logits, dim=-1)
            teacher_preds = torch.argmax(probs, dim=1)

            teacher_preds_one_hot = [] #独热编码 教师预测
            ...
            if args.use_hard_distil:
                #以下model为学生模型
                #在student_codeber_model中需要加入groups来区分。
            ...
            total_loss = args.alpha * loss + (1-args.alpha) * loss_dis#公式3、4，求软蒸馏或者硬蒸馏
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        input_ids, labels, groups = [x.to(args.device) for x in batch]        
        with torch.no_grad():
            probs, betas = model(input_ids=input_ids,groups=groups)
...
def test(args, model, tokenizer, test_dataset, beta):
    ...
    for batch in test_dataloader:
        input_ids, labels, groups = [x.to(args.device) for x in batch]       
        with torch.no_grad():
            probs = model(input_ids=input_ids, best_beta=BEST_BETA, groups=groups)
    ...
    pre = precision_score(y_trues, y_preds, average='weighted')
    rec= recall_score(y_trues, y_preds, average='weighted')
    result = {
